...
```
/login （メールを入力）
  ↓ supabase.auth.signInWithOtp()  … マジックリンクを送信（登録の有無で出し分けない）
  ↓ （メールのリンクを開く）
/auth/callback
...
#### 在籍オラクルの解消（`/api/auth/check-email` は廃止）

以前は送信前に `/api/auth/check-email` で登録済みかを確認し、未登録なら
「このメールアドレスは登録されていません」と表示していた。これは未認証で誰でも叩けたため、
**任意の人物について「この研修の受講者か」を確認できる**状態だった（本人の雇用状況に関わる情報で、
企業のメール形式は推測しやすいため受講者名簿の再構成も現実的だった）。

このAPIは削除し、`/login`・`/register` は**登録の有無にかかわらず同じ画面**を出す。

> 「〇〇 が登録されていれば、リンクをお送りしています」

未登録のアドレスにリンクが届いても、`/api/auth/link` が 403 を返すのでログインは成立しない。
打ち間違えた人が迷子にならないよう、送信完了画面には
「心当たりがない場合・届かない場合は事務局へ」の一文を必ず添える。

#### なぜ `shouldCreateUser: false` にしないのか

一見すると未登録者にリンクを送らずに済むが、**招待済みでまだ一度もログインしていない受講者は
`auth.users` に存在しない**。この設定にすると、その人たちにリンクが届かなくなり、
永久にログインできない。ゲートは `auth.users` ではなく `public.users` で掛ける、が正しい。
