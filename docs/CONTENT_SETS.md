...
コホートは案件受注ごとに1つずつ増える。増やすたびに、データセットのガイドラインから
外れていないかを自動でチェックする。仕組みは2段構え:

- **errors（データ契約違反）** … 比較不能になる致命的な逸脱。`npm run content:check` が
  非ゼロ終了し、**`npm run build` もここで止まる**（デプロイに乗らない）。
  例: 正典に無いキーを使った / 既存キーの文言を変えた / 事後スケールが事前に無い。
- **warnings（ガイドライン逸脱の指摘）** … ブロックはしない指摘。意図的なら残してよい。
  例: 比較の中核 §A・§C を省いた / 外部ベンチマーク §D を省いた。

チェックが走るタイミング:

| タイミング | 何が起きるか |
|-----------|-------------|
| 開発サーバー（非本番）起動時 | セット読込時に自動検査。errors があれば例外で即発覚 |
| `npm run content:check` | 全セットを検査。✓ / ⚠（指摘）/ ✗（違反）を表示。✗ があれば非ゼロ終了 |
| `npm run build`（＝デプロイ） | 上記チェックを内包。✗ があればビルド失敗 |

新しいコホートは `lib/content/sets/_TEMPLATE.ts` をコピーして始めると、ルールに沿った
雛形から書ける。
